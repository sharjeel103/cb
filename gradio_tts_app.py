...
SAFE_BATCH_SIZE = 4         # Fallback if OOM occurs
...
def load_model():
    print(f"Loading Genuine ChatterboxTTS on {DEVICE}...")
...
def generate(
    ...
    # --- 2. DYNAMIC BATCH CONFIG ---
    if cfg_weight > 0.0:
        target_batch_size = default_batch_size  # Slower, High Quality (VRAM Intensive)
        print(f"CFG Enabled ({cfg_weight}). Using Batch Size: {target_batch_size}")
    else:
        target_batch_size = 18  # Faster, Standard Quality
        print(f"CFG Disabled (0.0). Using Batch Size: {target_batch_size}")
    ...
    with torch.inference_mode():
        while chunk_idx < total_chunks:
            ...
            try:
                ...
                current_batch_size = target_batch_size # Reset to optimal
...
with gr.Blocks(title="Chatterbox Genuine", css="") as demo:
    gr.Markdown("# 💬 Chatterbox Genuine (Robust)")
    model_state = gr.State(None)
    
    with gr.Row():
        with gr.Column():
            text = gr.Textbox(
                value="The history of artificial intelligence is a fascinating journey that begins not with computers, but with the philosophical musings of ancient thinkers.",
            ...
            # [GENUINE PARAMS]
            exaggeration = gr.Slider(0.25, 2.0, step=0.05, label="Exaggeration (Neutral = 0.5)", value=0.5)
            # [CFG DEFAULT 0.0]
            cfg_weight = gr.Slider(0.0, 1.0, step=0.05, label="CFG Weight (0.0 = Fast Mode, >0.0 = Quality Mode)", value=0.0)
        ...
        with gr.Column():
            ...
            with gr.Accordion("Advanced Options", open=False):
                ...
                top_p = gr.Slider(0.00, 1.00, step=0.01, label="Top P", value=1.0)
                top_k = gr.Slider(0, 1000, step=10, label="Top K", value=1000)
                min_p = gr.Slider(0.00, 1.00, step=0.01, label="Min P", value=0.05)
